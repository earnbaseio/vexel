...
export const apiAuth = {
  ...
  async signIn(email: string, password: string): Promise<ITokenResponse> {
    const res = await fetch(`${apiCore.url}/auth/sign-in`, {
      method: "POST",
      body: JSON.stringify({
        email,
        password,
  ...
  async signUp(email: string, password: string, fullName?: string): Promise<ITokenResponse> {
    const res = await fetch(`${apiCore.url}/auth/sign-up`, {
      method: "POST",
      body: JSON.stringify({
        email,
        password,
  ...
  async refreshToken(token: string): Promise<ITokenResponse> {
    const res = await fetch(`${apiCore.url}/auth/refresh`, {
      method: "POST",
      headers: apiCore.headers(token),
    });
    return (await jsonify(res)) as ITokenResponse;
  },
  async getCurrentUser(token: string): Promise<IUserProfile> {
    const res = await fetch(`${apiCore.url}/auth/me`, {
      method: "GET",
      headers: apiCore.headers(token),
    });
  ...
  // LEGACY LOGIN WITH MAGIC LINK OR OAUTH2 (USERNAME/PASSWORD)
