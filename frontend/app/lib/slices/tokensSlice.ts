    ...
    if (hasAccessTokenExpired) {
      ...
      if (!hasRefreshTokenExpired) {
        try {
          ...
          dispatch(setTokens(response));
          return response; // Return the new tokens
        } catch (error) {
          dispatch(deleteTokens());
          throw new Error("Failed to refresh token - API call failed");
        }
      } else {
        dispatch(deleteTokens());
        throw new Error("Failed to refresh token - refresh token expired");
      }
    }
    // Token is still valid, return current state
