...
// NEW UNIFIED AUTH FUNCTIONS
export const signIn = (payload: { email: string; password: string }) =>
  async (dispatch: ThunkDispatch<any, void, Action>, getState: () => RootState) => {
    try {
      console.log('🔐 Attempting sign-in with new auth API...');

      // Call new auth API
      const response = await apiAuth.signIn(payload.email, payload.password);
      ...
      dispatch(addNotice({
        ...
        content: error.message || "Please check your credentials and try again.",
        icon: "error",
      }));
      dispatch(logout());
      throw error;
...
export const signUp = (payload: { email: string; password: string; fullName?: string }) =>
  async (dispatch: ThunkDispatch<any, void, Action>, getState: () => RootState) => {
    try {
      console.log('📝 Attempting sign-up with new auth API...');

      // Call new auth API
      const response = await apiAuth.signUp(payload.email, payload.password, payload.fullName);
      ...
      dispatch(addNotice({
        ...
        content: error.message || "Please check your details and try again.",
        icon: "error",
      }));
      throw error;
    }
...
// LEGACY LOGIN FUNCTION (keep for backward compatibility)
